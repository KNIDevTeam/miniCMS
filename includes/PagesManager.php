...
class PagesManager extends BaseClass
{
    ...
    public function __construct()
    {
        ...
        $this->currentPageName = $this->request->path;
    ...
    private function generateMenu($pages)
    {
        ...
        foreach ($pages as $pageName => $page) {
            $menu .= '<li class="item top-item"><a href="'.$this->request->baseUrl.$pageName.'" class="menu-link top-menu-link '.($this->currentPageName == strtolower($pageName) ? 'active' : '').'">'.str_replace('_', ' ', $pageName).'</a>';
    ...
    private function generateSubMenu($pages)
    {
        ...
        foreach ($pages as $pageName => $page) {
            $menu .= '<li class="item"><a href="'.$this->request->baseUrl.$pageName.'" class="menu-link '.($this->currentPageName == strtolower($pageName) ? 'active' : '').'">'.str_replace('_', ' ', $pageName).'</a>';
            if (isset($page['children']))
                $menu .= $this->generateSubMenu($page['children']);
            $menu .= '</li>';
        }
